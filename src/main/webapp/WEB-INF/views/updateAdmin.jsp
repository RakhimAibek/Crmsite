                ...
                <div class="card-body">
                    <form action="/updateAdmin", method="post">
                        <table id="myAreaChart" width="100%">
                            ...
                            <tr>
                                <td>Change login:<br>
                                    <input type="hidden" name="adminId" value="<%if(user!=null){out.print(user.getId());}%>"></td>
                                    <input type="text" name="login" value="<%if(user!=null){out.print(user.getLogin());}%>"></td>
                            </tr>

                            <tr>
                                <td>Change password:<br>
                                    <input type="text" name="password" value="<%if(user!=null){out.print(user.getPassword());}%>"></td>
                            </tr>

                            <tr>
                                <td>Change name:<br>
                                    <input type="text" name="name" value="<%if(user!=null){out.print(user.getName());}%>"></td>
                            </tr>

                            <tr>
                                <td>Change surname:<br>
                                    <input type="text" name="surname" value="<%if(user!=null){out.print(user.getSurname());}%>"></td>
                            </tr>
                            <%if(user!=null) {%>
                            <tr>
                                <td>Select company for admin:<br>
                                    <select name="cabinetName">

                                        <option value="<%if(user!=null){out.print(user.getCabinet_id().getName());} %>" selected><% if(user!=null){out.print(user.getCabinet_id().getName());}%></option>
                                        <%
                                            List<Cabinets> cabinetsList = (List<Cabinets>)request.getAttribute("allCabinets");

                                            if(cabinetsList!=null){

                                                for (Cabinets cabinet: cabinetsList){
                                                    if (cabinet.getActive()==1 && !cabinet.getName().equals(user.getCabinet_id().getName())) {
                                        %>

                                                     <option value="<% out.print(cabinet.getName());%>"><% out.print(cabinet.getName());%></option>
